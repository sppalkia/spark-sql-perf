...
val workers: Int = 1 //number of nodes, assumes one executor per node
...
val numExecutors: Int = sc.getExecutorMemoryStatus.size - 1
def numWorkers: Int = if (numExecutors > 0) numExecutors else 1 // In local mode 1 worker
...
cd tpch-dbgen
$checkoutRevision
sed -i.bu 's|malloc.h|malloc/malloc.h|g' bm_utils.c
sed -i.bu 's|malloc.h|malloc/malloc.h|g' varsub.c
make
...
def createExternal(location: String, dbname: String, tables: Tables) = {
  tables.createExternalTables(location, fileFormat, dbname, overwrite = overwrite, discoverPartitions = partitionTables)
}
